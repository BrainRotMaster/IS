...
namespace AForge.WindowsForms
{
    public class TelegramService : IDisposable
    {
        ...
        public TelegramService(string token, AIMLService aimlService)
        {
            ...
            // Достаем имя бота через новый метод GetMe()
        ...
        {
            MessageBox.Show("TG service handle " + client.Token);
            ...
            if (message.Type == MessageType.Photo)
            {
                ...
                if (photo != null)
                {
                    ...
                    var tgFile = await botClient.GetFile(photo.FileId, cancellationToken);

                    var imageStream = new MemoryStream();
                    await botClient.DownloadFile(tgFile.FilePath, imageStream, cancellationToken);
            ...
            {
                await client.SendMessage(message.Chat.Id, aiml.Talk(chatId, username, "Видео"), cancellationToken: cancellationToken);
                return;
            }
            if (message.Type == MessageType.Audio)
            {
                await client.SendMessage(message.Chat.Id, aiml.Talk(chatId, username, "Аудио"), cancellationToken: cancellationToken);
                return;
            }
        }
