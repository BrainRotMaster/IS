    ...
    internal class MagicEye
    {
        ...
        public bool ProcessImage(Bitmap bitmap)
        {
            ...
            if (settings.processImg)
            {
                ...
                Sample s = dataset.getSample(processed);
                currentType = network.Predict(s);
        ...
        private string processSample(ref Imaging.UnmanagedImage unmanaged)
        {
            ...
            //почему то считает на 1 блоб больше, вероятно связано с этой вотермаркой?
            Rectangle[] rects = bc.GetObjectsRectangles();
            rez = "Насчитали " + (rects.Length - 1).ToString() + " прямоугольников!";
            //if (rects.Length == 0)
            //{
            ...
            {
                lx = System.Math.Min(lx, rects[i].Left);
                ly = System.Math.Min(ly, rects[i].Top);
                rx = System.Math.Max(rx, rects[i].Right);
                ry = System.Math.Max(ry, rects[i].Bottom);
            }
            ...
            // 3. Квадрат вокруг центра
            int newLx = cx - size / 2;
            int newLy = cy - size / 2;

            // 4. Ограничение границ изображения
            if (newLx < 0) newLx = 0;
            if (newLy < 0) newLy = 0;

            if (newLx + size > unmanaged.Width)
                size = unmanaged.Width - newLx;

            if (newLy + size > unmanaged.Height)
                size = unmanaged.Height - newLy;

            // 5. Обрезка
            Rectangle roi = new Rectangle(newLx, newLy, size, size);
            AForge.Imaging.Filters.Crop cropFilter =
                new AForge.Imaging.Filters.Crop(roi);
